...
<div class="custom-lightbox-wrapper">
    <?php
        ...
        switch ($settings['trigger_type']) {
            case 'button':
                echo '<button class="lightbox-trigger" data-target="#' . esc_attr( $unique_id ) . '">' . esc_html( $settings['button_text'] ) . '</button>';
                break;
            case 'image':
                echo '<img class="lightbox-trigger" data-target="#' . esc_attr( $unique_id ) . '" src="' . esc_url( $settings['trigger_image']['url'] ) . '" alt="Lightbox Trigger" style="cursor: pointer;">';
                break;
            case 'text':
                echo '<a href="#" class="lightbox-trigger" data-target="#' . esc_attr( $unique_id ) . '" style="cursor: pointer;">' . esc_html( $settings['trigger_text'] ) . '</a>';
                break;
        }
    ?>  
    <!-- Lightbox Content (hidden by default) -->
    <div id="<?php echo esc_attr( $unique_id ); ?>" class="custom-lightbox-content" style="display: none;">
        <div class="lightbox-inner">
            ...
            switch ($settings['lightbox_type']) {
                ...
                case 'text':
                    ...
                    break;
                case 'video':
                    $video_url = $settings['video_content'];
                    if (strpos($video_url, 'youtube') !== false || strpos($video_url, 'youtu.be') !== false) {
                        // YouTube video
                        preg_match('/(?:youtube\.com\/(?:[^\/]+\/.+\/|(?:v|e(?:mbed)?)\/|.*[?&]v=)|youtu\.be\/)([^"&?\/\s]{11})/', $video_url, $matches);
                        $video_id = isset( $matches[1] ) ? sanitize_text_field( $matches[1] ) : '';
                        ...
                        $video_id = isset( $matches[2] ) ? sanitize_text_field( $matches[2] ) : '';
                        echo '<iframe src="https://player.vimeo.com/video/' . esc_attr( $video_id ) . '" width="640" height="360" frameborder="0" allowfullscreen></iframe>';
                    } else {
                        // Other video (MP4, etc.)
